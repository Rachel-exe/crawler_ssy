...
var topicUrls = [
  ...
  'http://tad.tjfsu.edu.cn/class.asp'
];
...
function fetchData(includeWords, excludeWords, cb) {
  ...
  requestOpts.forEach(function (opt) {
    request(opt, function (error, response, body) {
      ...
      var items = [];
      $('.articlelist li').each(function (index, el) {
        var title = $('.links>font', el).text();
        // console.log(title);
        if (checkIncludeWords(title, includeWords) && checkExcludeWords(title, excludeWords)) {
          var item = {
            title   : title,
            href    : $('.links', el).attr('href'),
            lastTime: $('span', el).text()
          };
  ...
  urls.forEach(function (url) {
    for (var i = 0; i < num; i++) {
      opts.push({
        ...
        headers: {
          'Accept'       : '*/*',
          'Accept-Language': 'utf-8,gbk,zh-CN,zh;q=0.8',
